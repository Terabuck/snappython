...
class Command(BaseCommand):
    ...
    def process_video(self, filename):
        ...
        try:
            ...
            compound_image = self.build_horizontal_compound(aligned_frames)
    ...
    def build_horizontal_compound(self, frames):
        ...
        # Create canvas with black space at top
        top_padding = 100
        canvas_height = height + top_padding
        canvas_width = width * 3  # Wide enough for compounding
        
        # Fix 1: Consistent canvas dimensions for grayscale/color
        if is_color:
            canvas = np.zeros((canvas_height, canvas_width, 3), dtype=np.uint8)
        else:
            # Grayscale needs 3D canvas (H, W, 1)
        ...
        if is_color:
            ...
            # Fix 2: Correct grayscale assignment to 3D canvas
        ...
        for i in range(1, len(frames)):
            ...
            # Create blending mask only if dimensions are positive
            h = frame_y2 - frame_y1
            w = frame_x2 - frame_x1
            if h <= 0 or w <= 0:
                self.stderr.write("Warning: Skipping frame due to negative dimensions")
                continue
                
            blend_mask = np.ones((h, w), dtype=np.float32)
            blend_width = min(overlap_width, frame_x2 - frame_x1)
            ...
            if is_color:
                ...
                # Fix 3: Handle grayscale blending with 3D canvas
                canvas_region = canvas[y_start:y_end, x_start:x_end]
                frame_region = frame_shifted[frame_y1:frame_y2, frame_x1:frame_x2]
        ...
        # Fix 4: Unified axis handling for color/grayscale cropping
