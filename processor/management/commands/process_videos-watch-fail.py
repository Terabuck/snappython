...
class Command(BaseCommand):
    ...
    def extract_frames(self, video_path, temp_dir, num_frames=15):
        """Extract the last 'num_frames' frames with robust error handling"""
        # Ensure temp directory exists
        os.makedirs(temp_dir, exist_ok=True)
        
        # Get total frame count with error handling
        try:
            cmd = [
                'ffprobe', '-v', 'error', '-select_streams', 'v:0',
                '-show_entries', 'stream=nb_frames', 
                '-of', 'default=nokey=1:noprint_wrappers=1', 
                f'"{video_path}"'  # Quote the path
            ]
        ...
        # Extract frames with proper path quoting
        cmd = (
            f'ffmpeg -i "{video_path}" '
            f'-vf "select=gte(n\\,{start_frame})" '
            f'-vframes {num_frames} '
            f'-vsync 0 '
            f'"{os.path.join(temp_dir, "frame_%04d.png")}"'
        )
        ...
        try:
            ...
            if result.returncode != 0:
                self.stderr.write(f"FFmpeg error: {result.stderr}")
        except Exception as e:
            self.stderr.write(f"Frame extraction failed: {str(e)}")
        
        # Load whatever frames were extracted
        return self.load_available_frames(temp_dir)

    def load_available_frames(self, temp_dir):
        """Load whatever frames were successfully extracted"""
        frames = []
        has_color = False
        ...
        try:
            ...
            if not frame_files:
                self.stderr.write("No frames extracted")
                return [], False
            ...
            self.stderr.write(f"Error loading frames: {str(e)}")
