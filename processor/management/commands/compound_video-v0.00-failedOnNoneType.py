...
class Command(BaseCommand):
    ...
    def process_video(self, filename):
        ...
        try:
            # Extract frames (more frames for better compounding)
    ...
    def detect_and_crop_ultrasound(self, frames):
        """Detect and crop the ultrasound window from frames using edge detection and contour analysis"""
    ...
    def apply_vertical_shift(self, frame, dy):
        ...
        if dy > 0:
            # Shift downward
            new_frame = np.zeros((new_height, width, channels) if channels > 1 else (new_height, width), dtype=frame.dtype)
            new_frame[dy:, :] = frame
        else:
            # Shift upward
            new_frame = np.zeros((new_height, width, channels) if channels > 1 else (new_height, width), dtype=frame.dtype)
    ...
    def enhance_unified(self, img_pil):
        """Apply unified enhancement pipeline with surface line preservation"""
        ...
        # Use edge detection to preserve important boundaries
